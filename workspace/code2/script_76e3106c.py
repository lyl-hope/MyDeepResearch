...
# 性能指标
categories = ['加速性能', '续航能力', '车身尺寸', '轴距空间']
N = len(categories)

# 数据归一化（0-1范围）
accel_norm = [1 - (p - min(performance['零百加速(s)'])) / (max(performance['零百加速(s)']) - min(performance['零百加速(s)'])) 
              for p in performance['零百加速(s)']]
range_norm = [(p - min(performance['CLTC续航(km)'])) / (max(performance['CLTC续航(km)']) - min(performance['CLTC续航(km)'])) 
              for p in performance['CLTC续航(km)']]
length_norm = [(p - min(performance['车身长度(mm)'])) / (max(performance['车身长度(mm)']) - min(performance['车身长度(mm)'])) 
              for p in performance['车身长度(mm)']]
wheelbase_norm = [(p - min(performance['轴距(mm)'])) / (max(performance['轴距(mm)']) - min(performance['轴距(mm)'])) 
                  for p in performance['轴距(mm)']]

# 创建雷达图数据
values_su7 = [accel_norm[0], range_norm[0], length_norm[0], wheelbase_norm[0]]
values_yu7 = [accel_norm[1], range_norm[1], length_norm[1], wheelbase_norm[1]]
...
with open(text_path, 'w', encoding='utf-8') as f:
    ...
    f.write(f"   车身尺寸: 两者相近，YU7略高更宽\n\n")
    
    f.write("3. 配置特点:\n")
    f.write("   小米SU7: 运动轿跑定位，加速性能突出\n")
    f.write("   小米YU7: 豪华SUV定位，800V平台技术先进\n\n")
    
    f.write("4. 智能配置:\n")
    f.write("   两者均标配激光雷达和700TOPS算力芯片\n")
    f.write("   SU7使用Xiaomi HAD系统，YU7使用澎湃OS系统\n")
